...
public class StatusBarIndicator extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (verticalLinePositions.size() > 0 && maskWhiteSegment) {
            maskWhiteSegment = false;
            horizontalPaint.setColor(Color.BLACK);
            canvas.drawLine(verticalLinePositions.get(verticalLinePositions.size() - 1), 0, getWidth(), 0, horizontalPaint);//dodo da zamaskira bijeli visak
        }
        if (markLastSegment) {
